...
def test_offline_sample_availability_ignores_mutable_runtime_samples(
    settings_factory,
    tmp_path,
    monkeypatch,
):
    runtime = tmp_path / "runtime"
    sample = runtime / "dcr_output_RUNTIME-ONLY" / "shuffled_sample.csv"
    sample.parent.mkdir(parents=True)
    sample.write_text("age\n70\n", encoding="utf-8")
    settings = settings_factory(
    ...
    assert result == {
        ...
        "cohorts_without_samples": ["RUNTIME-ONLY"],
    }
