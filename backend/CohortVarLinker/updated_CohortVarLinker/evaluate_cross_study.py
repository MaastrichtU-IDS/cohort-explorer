...
from llm.utils import clean_label_remove_temporal_context
import logging
...
def load_dictionaries(cohorts_dir: str) -> dict[str, dict[str, str]]:
    ...
    for entry in sorted(os.listdir(cohorts_dir)):
        ...
        for _, r in df[['variablename', 'variablelabel']].iterrows():
            name  = str(r['variablename']).strip().lower()
...
def _build_name_to_canonical_label(pred_df: pd.DataFrame) -> tuple[dict, dict]:
    """From raw prediction file, build {name: canonical_label} for source and target."""
    s_map, t_map = {}, {}
    for _, r in pred_df.iterrows():
        s  = str(r['source']).strip().lower()
        t  = str(r['target']).strip().lower()
        sl = clean_label_remove_temporal_context(str(r.get('source_label', '') or '').strip().lower()) or s
        tl = clean_label_remove_temporal_context(str(r.get('target_label', '') or '').strip().lower()) or t
        if s in s_map and s_map[s] != sl:
            logger.warning(f"source label conflict for '{s}': '{s_map[s]}' vs '{sl}' — keeping first")
        else:
            s_map.setdefault(s, sl)
        if t in t_map and t_map[t] != tl:
            logger.warning(f"target label conflict for '{t}': '{t_map[t]}' vs '{tl}' — keeping first")
        else:
            t_map.setdefault(t, tl)
    return s_map, t_map
...
def _collapse_by_label(df, label_col, s_map, t_map, rank=None):
    rank = rank or {'identical match':0,'complete match':1,'compatible match':1,'partial match':2,'not applicable':3}
    d = df.assign(
        src_key=df['src_var'].map(lambda x: s_map.get(x, x)),   # fallback: raw name
        tgt_key=df['tgt_var'].map(lambda x: t_map.get(x, x)),
        _rank=df[label_col].map(rank).fillna(99),
    )
    return (d.sort_values('_rank', kind='mergesort')
             .drop_duplicates(['src_key','tgt_key'], keep='first')
             .drop(columns='_rank')
             .rename(columns={'src_var':'src_var_orig','tgt_var':'tgt_var_orig',
                              'src_key':'src_var','tgt_key':'tgt_var'})
             .reset_index(drop=True)
             [['src_var','tgt_var','src_var_orig','tgt_var_orig',label_col]])
...
def load_predictions(file_path: str, s_map: dict, t_map: dict,
    ...
    pred = pd.DataFrame({
        'src_var':         p_df['source'].astype(str).str.strip().str.lower(),
        'tgt_var':         p_df['target'].astype(str).str.strip().str.lower(),
...
def load_ground_truth(file_path: str, source_study: str, target_study: str,
                      ...
                      collapse_temporal: bool = True) -> pd.DataFrame:
    """Read GT for a study pair. Uses name→label maps from predictions to canonicalize."""
    for enc in ('utf-8-sig', 'utf-8', 'latin-1', 'cp1252'):
        try:
            t_df = pd.read_csv(file_path, encoding=enc); break
        except UnicodeDecodeError:
            continue
    ...
    gt = pd.DataFrame({
        'src_var':       t_df.loc[mask, 'source_var_name'].astype(str).str.strip().str.lower(),
        'tgt_var':       t_df.loc[mask, 'target_var_name'].astype(str).str.strip().str.lower(),
...
def analyze_prediction_coverage(all_predictions: list, output_dir: str,
    ...
    summary_df.to_csv(os.path.join(output_dir, f'prediction_coverage_summary{tag}.csv'), index=False)
    unique_df.to_csv(os.path.join(output_dir, f'predictions_unique_to_mode{tag}.csv'), index=False)
...
# def run_comprehensive_evaluation(cohorts_dir:str, ground_truth_file: str, predictions_files: list, 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--models', nargs='+',
                        default=["biolord+no_llm","sapbert+no_llm", "biolord+gpt-oss-120b"
                        ],
