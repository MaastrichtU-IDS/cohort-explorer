...
#         pharmacological or clinical class.
#         """
#         if src == tgt:
#             return False
...
#         # ── Collect ancestors via is_a (directed) ────────────────
#         src_anc = self._bfs_dir(src_eq, max_hops, 'up')
#         tgt_anc = self._bfs_dir(tgt_eq, max_hops, 'up')

#         # Parent–child:one concept IS an ancestor of the other
#         if tgt_eq & (set(src_anc) | src_eq):
#             self._sibling_cache[key] = False
#             return False
#         if src_eq & (set(tgt_anc) | tgt_eq):
#             self._sibling_cache[key] = False
#             return False

#         # ── Fast check:raw ancestor overlap (same vocab) ────────
#         if set(src_anc) & set(tgt_anc):
#             self._sibling_cache[key] = True
#             return True

#         # ── Cross-vocab bridge:equiv-normalize ancestors ────────
#         src_anc_closures = set()
#         for a in src_anc:
#             src_anc_closures.add(id(self._equiv_closure(a)))
...
#         # both have same text term in concept_name
#         exact_term_match_targets = set()
#         start_name = self.get_node_attr(start, "concept_name").lower()
#         for tid in target_ids:
#             tid_name = self.get_node_attr(tid, "concept_name").lower()
#             if start_name == tid_name:
#                 exact_term_match_targets.add(tid)
#         return exact_term_match_targets
...
#         # ── Equivalence check (maps to / mapped from) ──
...
#         #         #  and not self.is_sibling_path(start, tid)
#         #         if tid in remaining and dist <= self._allowed_depth(vs, self.get_node_attr(tid, "vocabulary").lower(), max_depth):
#         #             # make match more explicit , Symbolic:broadMatch, Symbolic:narrowMatch
#         #             results.append((tid, "Symbolic:Graph Traversal"))
#         #         # sibling and loinc is not in any vocabulary
#         #         # elif self.is_sibling_path(start, tid) and 
#         #         #     results.append((tid, "Symbolic Match:Siblings"))
#         if remaining:
#             eq = self._equiv_closure(start)
...
#                 elif tid in self._sssp_lengths(start, cutoff=max_depth + 2):
#                     dist = self._sssp_lengths(start, cutoff=max_depth + 2)[tid]
#                     if dist <= allowed:
#                         # if self.is_sibling_path(start, tid):
#                         #     results.append((tid, "Symbolic:relatedMatch"))
#                         # else:
#                         results.append((tid, "Symbolic:closeMatch"))
#         return results
...
#         results, _ = self.source_to_targets_paths(sid, [tid], max_depth=max_depth)
...
#             for i in range(len(node_ids) - 1):
#                 u, v = node_ids[i], node_ids[i + 1]
#                 # Try direct edge in both directions
#                 rel = self.get_edge_rel(u, v)
#                 if not rel:
#                     rel = self.get_edge_rel(v, u)
#                 if not rel:
#                     # Check if it's an equiv relationship (cross-vocab mapping)
#                     if v in self._equiv_bidir.get(u, ()) or u in self._equiv_bidir.get(v, ()):
#                         rel = "equivalent"
#                     else:
#                         rel = "unknown"
#                 edges.append((u, v, rel))
...
#         # Phase 1: BFS through is_a/subsumes from each eq member independently,
#         #          tracking which eq member was the entry point.
#         visited = set(eq)
#         parent = {}            # node → predecessor in is_a/subsumes BFS
#         entry_point = {}       # node → which eq member it was reached from
#         q = deque()
#         for n in eq:
#             parent[n] = None   # eq members are roots of this BFS
#             entry_point[n] = n
#             q.append((n, 0))
...
#         # Phase 2: Reconstruct is_a/subsumes chain from tgt back to the eq entry point
#         hier_chain = []
#         cur = tgt
#         while cur is not None:
#             hier_chain.append(cur)
#             cur = parent.get(cur)
#         hier_chain.reverse()  # [eq_entry, ..., tgt]

#         # Phase 3: If entry point != src, prepend equiv BFS chain from src to entry
#         ep = hier_chain[0]
#         if ep == src:
#             return hier_chain

#         # BFS through equiv edges to connect src → ep
...
#         equiv_chain.reverse()  # [src, ..., ep]

#         # Merge: equiv_chain ends at ep, hier_chain starts at ep → skip duplicate
