...
@router.post("/replace_graph_pickle_file")
async def replace_graph_pickle_file(
    ...
    - Places the new file in the correct location
    - Returns confirmation with file details and size comparison
    """
    import pickle
    ...
    if file.filename != "graph_nx.pkl":
        raise HTTPException(
            status_code=400,
            detail=f"File must be named 'graph_nx.pkl', got '{file.filename}'"
        )
    
    # Import settings from CohortVarLinker to get the correct data folder
    try:
        from CohortVarLinker.src.config import settings as cvl_settings
        data_folder = cvl_settings.data_folder
    except ImportError:
        # Fallback to main settings if CohortVarLinker config is not available
    ...
    # Define file paths
    target_file_path = os.path.join(data_folder, "graph_nx.pkl")
    ...
    if os.path.exists(target_file_path):
        # Get old file stats
        old_file_stat = os.stat(target_file_path)
        old_file_size = old_file_stat.st_size
        old_file_modified = datetime.fromtimestamp(old_file_stat.st_mtime)
        
        # Create backup filename with timestamp
        timestamp_str = datetime.now().strftime("%Y%m%d_%H%M%S")
        backup_filename = f"graph_nx_pre_{timestamp_str}.pkl"
        backup_file_path = os.path.join(data_folder, backup_filename)
        
        # Rename old file to backup
        shutil.move(target_file_path, backup_file_path)
        
        response["old_file"] = {
            "name": backup_filename,
            "location": backup_file_path,
            "size_bytes": old_file_size,
            "size_mb": round(old_file_size / (1024 * 1024), 2),
            "last_modified": old_file_modified.isoformat()
        }
        
        response["size_change"] = {
            "bytes": len(file_content) - old_file_size,
            "mb": round((len(file_content) - old_file_size) / (1024 * 1024), 2),
            "percentage": round(((len(file_content) - old_file_size) / old_file_size) * 100, 2) if old_file_size > 0 else 0
        }
    else:
        response["old_file"] = None
        response["message"] = "No existing file found, creating new file"
    
    # Write the new file
    with open(target_file_path, "wb") as f:
        f.write(file_content)
    
    # Get the new file's last modified time
    new_file_stat = os.stat(target_file_path)
    response["new_file"]["last_modified"] = datetime.fromtimestamp(new_file_stat.st_mtime).isoformat()
    
    # Log the replacement
    logging.info(f"Graph pickle file replaced by {current_user.get('email', 'unknown')}")
