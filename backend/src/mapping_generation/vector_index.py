...
from .utils import exact_match_wo_vocab
from .data_loader import load_data
# from langchain.schema import Document
import argparse
from .utils import load_docs_from_jsonl,load_custom_docs_from_jsonl
...
def update_api_search_filter(api_retriever, domain='observation', topk=10):
    ...
    if domain == 'unit':
        ...
        api_retriever.filters  = AthenaFilters(vocabulary=['SNOMED','LOINC','OMOP Extension','RxNorm','ATC'], standard_concept=['Standard','Classification'])
...
def update_qdrant_search_filter(retriever, domain='all', topk=10):
    if domain == 'unit':
        ...
        retriever.search_kwargs['filter'] = rest.Filter(
                    must=[
                        rest.FieldCondition(
                            key="metadata.vocab",
                            match=rest.MatchAny(any=['rxnorm','rxnorm extension','atc','snomed'])
                        ),
                        rest.FieldCondition(
    ...
    elif domain == 'observation' or domain == 'visit' or domain == 'demographics' or domain == 'history of event' or domain == 'history of events' or domain == 'life style':
        retriever.search_kwargs['filter'] = rest.Filter(
                    must=[
                        rest.FieldCondition(
                            key="metadata.vocab",
        ...
        retriever.search_kwargs['filter'] = rest.Filter(
            ...
            must=[
                rest.FieldCondition(
                    key="metadata.vocab",
                    match=rest.MatchAny(any=['snomed','loinc','atc','rxnorm extension'])
                ),
                rest.FieldCondition(
                    key="metadata.is_standard",
                    match=rest.MatchAny(any=['S','C'])
...
def update_merger_retriever(merger_retriever:CustomCompressionRetriever, domain='all', topk=10) -> CustomCompressionRetriever:
    try:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--collection_name', type=str, default='concept_mapping', help='Generate vector index for given collection')
    parser.add_argument('--document_file_path', type=str, default='/workspace/mapping_tool/data/output/concepts.jsonl', help='Documents to index')
    parser.add_argument('--input_data', type=str, default='/workspace/mapping_tool/data/eval_datasets/custom_data/references.txt', help='Documents to index')
    parser.add_argument('--output_file', type=str, default='/workspace/mapping_tool/data/eval_datasets/results.txt', help='Documents to index')
