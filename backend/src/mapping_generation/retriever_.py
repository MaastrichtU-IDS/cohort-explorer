...
def temp_process_query_details(llm_query_obj:QueryDecomposedModel, retriever_cache:Any, 
    ...
    try:
        if llm_query_obj:
            ...
            variable_label_matches, additional_entities_matches,categories_matches,unit_matches = None, None, None, None
            ...
            logger.info(f"main_term={main_term}, context={additional_entities}, status={categories}, domain={domain}, unit={unit}")
            if additional_entities:
                logger.info(f"Processing additional entities: {additional_entities}")
                additional_entities_matches =  process_values(additional_entities, retriever_cache, llm_name, domain=domain, values_type='additional', is_omop_data=is_omop_data, topk=topk) if additional_entities else {}
            if categories:
                logger.info(f"Processing categories: {categories}")
                categories_matches =  process_values(categories,retriever_cache , llm_name, domain='all', values_type='status', is_omop_data=is_omop_data,topk=topk) if categories else {}
            # if rel:
            ...
            if unit:
                ...
                unit_matches =  process_unit(unit, retriever_cache, llm=llm_name, domain='unit',is_omop_data=is_omop_data, topk=topk) if unit else []
            mapping_result  = create_processed_result(ProcessedResultsModel(
    ...
    if docs and len(docs) > 0:
        ...
        if llm_name:
            ...
            if len(domain_specific_docs) == 0:
                return [], False
            llm_ranks, match_found =  pass_to_chat_llm_chain(query, domain_specific_docs, llm_name=llm_name,domain=domain, threshold=belief_threshold)
...
def process_values(values,retriever,llm, domain=None, values_type='additional',is_omop_data=False, topk=10):
    ...
    for q_value in values:
            ...
            if q_value and q_value != 'unknown':
                if categorical_value_results:=  retriever_docs(q_value, retriever, domain=domain, is_omop_data=is_omop_data, topk=topk):
                    ...
                    all_values[q_value] = [RetrieverResultsModel(standard_label='na', standard_code=None, standard_omop_id=None, vocab=None)]
    return all_values
