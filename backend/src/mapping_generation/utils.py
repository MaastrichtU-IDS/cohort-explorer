...
def save_to_csv(data, filename):
    ...
    fieldnames = [
        'VARIABLE LABEL', 'DOMAIN', 'Variable Concept Label', 'Variable Concept Code','Variable Concept OMOP ID',
...
def create_processed_result(result_object:ProcessedResultsModel) -> dict:
    ...
    additional_entities_labels, additional_entities_codes, additional_entities_omop_ids  = format_categorical_values(additional_entities_matches,additional_entities)
    categorical_values_labels, categorical_values_codes, categorical_values_omop_ids = format_categorical_values(categorical_values_matches,categorical_values)
    ...
    results =  {
        ...
        'Unit Concept Label': result_object.unit_matches[0].standard_label if len(result_object.unit_matches)  >= 1 else None,
        'Unit Concept Code': result_object.unit_matches[0].standard_code if len(result_object.unit_matches)  >= 1 else None,
        'Unit OMOP ID':  result_object.unit_matches[0].standard_omop_id if len(result_object.unit_matches)  >= 1 else None
    }
...
def format_categorical_values(values_matches_documents: Dict[str, List[RetrieverResultsModel]], values_list: List[str]) -> Tuple[str, str, str]:
    labels = []
    codes = []
    ids = []
...
def parse_tokens(text: str, semantic_type: bool = False, domain: str = None) -> Tuple[str, List[str], str]:
    ...
    if 'concept name:' in text_ and 'synonyms:' in text_ and 'domain:' in text_ and 'concept class:' in text_ and 'vocabulary:' in text_:
        ...
        global_logger.info(f"Entity: {entity}, Synonyms: {synonyms}, Semantic Type of Entity: {semantic_type_of_entity}")
    else:
...
def append_results_to_csv(input_file, results, output_suffix='_mapped.csv') -> None:
    ...
    new_columns_data = {
        ...
        'Additional Context Concept Label': [result.get('Additional Context Concept Label', None) for result in results],
        ...
        'Primary to Secondary Context Relationship': [result.get('Primary to Secondary Context Relationship', None) for result in results],
        'Categorical Values Concept Label': [result.get('Categorical Values Concept Label', None) for result in results],
        'Categorical Values Concept Code': [result.get('Categorical Values Concept Code', None) for result in results],
        'Categorical Values Concept OMOP ID': [result.get('Categorical Values Concept OMOP ID', None) for result in results],
