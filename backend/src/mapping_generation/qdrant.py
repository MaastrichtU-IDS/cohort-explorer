        ...
        **kwargs: Any,
        ...
        for metadata in metadatas or [{}]:  # Default to empty dict if metadatas is None
            if "synonyms" in metadata:
                ...
                formatted_text = (
                    f"<ent>{metadata.get('label', 'unknown')}</ent><domain>{metadata.get('domain', 'general')}</domain>"
                )

            formatted_texts.append(formatted_text)

        # Now use the base class's batch generation to add the vectors
        for batch_ids, points in tqdm(self._generate_batches(formatted_texts, metadatas, ids, batch_size)):
