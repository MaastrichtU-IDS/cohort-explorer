...
,'function','fn','fnc','pst','panl','ob','gyn','p birth','birth','pan','panel.','proc','procedure','device','stnd','standard',';;',';','+','-','_',
                '"' ,'ql','qn',
}
...
class Omop:
    def __init__(self, concept_dir, taxonomy=True):   
        ...
        self.omop_rel_path=  os.path.join(concept_dir, 'output/relationships.csv') #change it with ancestor_concept.csv which include ancestor_concept_id, decendant_concept_id, min_levels_of_seperation, max_levels_of_seperation, consider min_level which could be 0 or 1 : 0 in cases where ancestor_concept_id can be its own ancestor
    ...
    def build_graph_for_synonym_pairs(self):
                ...
                if not pd.isna(row['concept_name']) and row['concept_name'] != 'nan':
                    ...
                    if not pd.isna(row['concept_synonym_name']):
                        if ';;' in row['concept_synonym_name']:
                            synonyms = self.filter_synonyms(cname, vocab=ontology_, entity_synonyms= set(row['concept_synonym_name'].split(';;')), stop_words=STOP_WORDS)
                        else:
                    ...
                    if desc:
                        self.graph.add_node(cid, code=code_, desc=desc, synonyms=synonyms,hier_syn=hier_synonyms,maps_to_syn=maps_to_syn,
                                            aligned_synonyms=aligned_synonyms,
                                            domain=domain_,concept_class=concept_class,ontology=ontology_, 
                                            type_standard=type_standard, has_answers = [], answer_of =_answer_of)
        ...
        for cid in self.graph.nodes():
            ...
            elif len(node['aligned_synonyms']) > 0:
                updated_vocab_synonym_counts[ontology_] += 1
            elif len(node['hier_syn']) > 0:
                updated_vocab_synonym_counts[ontology_] += 1
