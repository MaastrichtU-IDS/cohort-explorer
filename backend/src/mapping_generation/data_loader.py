...
from  .utils import save_docs_to_jsonl, load_docs_from_jsonl, normalize, global_logger as logger
import datetime
from .param import *
            ...
            if input_file.endswith('.csv'):
                with open(input_file, mode='r', encoding='utf-8') as file:
                    ...
                    for row in reader:
                        ...
                        queries.append(key, QueryDecomposedModel(
                            full_query=mention,
                            base_entity=mention,
                            categories=[],
                            unit=None,
                ...
                with open(input_file, 'r', encoding='utf-8') as file:
                    for line in tqdm(file,desc='Loading queries from input file'):
                        ...
                        if len(parts) == 2:
                            ...
                            if cui == 'cui-less': continue
                            queries.append(cui, QueryDecomposedModel(
                                        full_query=query,
                                        base_entity=query,
                                        categories=[],
                                        unit=None,
