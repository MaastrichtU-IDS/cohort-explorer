...
def check_concept_name_duplicates(concept_df):
    ...
    result_df = pd.concat(
        [
            ...
            duplicates_c,
            concept_df[
                ~concept_df["concept_name"].isin(duplicates["concept_name"]) & (concept_df["standard_concept"] == "C")
            ],
        ]
    )
...
def fetch_concept(
    data_directory,
    only_standard=True,
    vocabulary_list=["Korean Revenue Code", "Concept Class", "Domain", "Metadata", "Vocabulary", "Notes"],
    sep="\t",
    output_dir=DATA_DIR,
):
    ...
    if "concept_synonym_name" in concept_df.columns:
        concept_df["concept_synonym_name"] = concept_df.apply(
            ...
            if row["concept_synonym_name"] and row["synonym"]
                ...
                row["concept_synonym_name"].strip(";;") if row["concept_synonym_name"] else row["synonym"].strip(";;")
            ),
            axis=1,
        )
    else:
...
def fetch_synonyms(file, concept_df, output_dir=DATA_DIR):
    ...
    # Group by concept_id
    grouped_synonyms = (
        english_synonyms.groupby("concept_id")["concept_synonym_name"].apply(lambda x: ";;".join(x)).reset_index()
    )
    ...
    vocab_stats = (
        concept_df.groupby("vocabulary_id")
        .agg({"concept_id": "count", "concept_synonym_name": lambda x: x.apply(lambda y: bool(y)).sum()})
        .reset_index()
    )
    ...
    for _, row in vocab_stats.iterrows():
        print(
            f"{row['vocabulary_id']}: {row['percentage']:.2f}% ({row['concept_synonym_name']} out of {row['concept_id']} concepts)"
        )
    ...
    if "concept_synonym_name" in concept_df.columns:
        concept_df["concept_synonym_name"] = concept_df.apply(
            ...
            if row["concept_synonym_name"] and row["synonym"]
                ...
                str(row["concept_synonym_name"]).strip(";;")
                if row["concept_synonym_name"]
                else str(row["synonym"]).strip(";;")
            ),
            axis=1,
        )
    else:
...
def update_relationships(file, concept_df, relationship_names, data_dir=DATA_DIR):
    ...
    relationship_df = relationship_df[
        (relationship_df["concept_id_1"].isin(concept_ids)) & (relationship_df["concept_id_2"].isin(concept_ids))
    ]
    print(f"Rel={relationship_df.shape}")
    ...
    relationship_name_map = pd.Series(
        relationship_names["relationship_name"].values, index=relationship_names["relationship_id"]
    ).to_dict()
...
def main_process(data_directory="/workspace/mapping_tool/data/omop/ALL_VOCAB_29_07_2024", output_dir=DATA_DIR):
    ...
    concepts_df = fetch_synonyms(
        os.path.join(data_directory, "CONCEPT_SYNONYM.csv"), concepts_df, output_dir=output_dir
    )
    relationship_names = pd.read_csv(
        os.path.join(data_directory, "RELATIONSHIP.csv"), sep="\t", low_memory=False, dtype=str
    )
    print(f"unique relationships = {relationship_names['relationship_id'].unique().tolist()}")
    update_relationships(
        os.path.join(data_directory, "CONCEPT_RELATIONSHIP.csv"), concepts_df, relationship_names, data_dir=output_dir
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dir",
        type=str,
        default="/workspace/mapping_tool/data/omo/ALL_VOCAB_29_07_2024",
        help="Directory containing the vocabulary data files",
    )
    parser.add_argument(
        "--output_dir", type=str, default=DATA_DIR, help="Directory containing the vocabulary data files"
    )
