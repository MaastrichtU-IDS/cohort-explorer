...
def _get_embeddings_from_stateful_docs(
    ...
    if len(documents) and "embedded_doc" in documents[0].state:
        ...
        embedded_documents = embeddings.embed_documents([d.metadata["label"] for d in documents])
...
class MyEmbeddingsFilter(EmbeddingsFilter):
    ...
    def compress_documents(
        ...
        try:
            ...
            raise ImportError("To use please install langchain-community " "with `pip install langchain-community`.")
        ...
        sorted_docs_with_scores = sorted(
            [(stateful_documents[i], similarity[i]) for i in included_idxs], key=lambda x: x[1], reverse=True
        )
        # Remove state information from the documents
        compressed_documents = [
            Document(page_content=doc.page_content, metadata=doc.metadata) for doc, _ in sorted_docs_with_scores
        ][:10]
