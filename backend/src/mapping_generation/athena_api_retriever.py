...
class AthenaFilters(BaseModel):
    ...
    domain: list[str] | None = Field(default=None, example=['Condition', 'Drug', 'Measurement', 'Observation','Device','Meas Value'])
    standard_concept: list[str] | None = Field(default=None, example=['Standard', 'Classification', 'Non-standard'])
    vocabulary: list[str] | None = Field(default=None, example=['SNOMED', 'LOINC', 'MeSH', 'UCUM', 'OMOP Extension', 'ATC', 'RxNorm','Gender','Race','Ethnicity'])
    ...
    def __init__(self, **data):
        super().__init__(**data)
        self.conceptClass = [self.validate_and_format(item) for item in self.conceptClass] if self.conceptClass else None
        self.domain = [self.validate_and_format(item) for item in self.domain] if self.domain else None
        self.standard_concept = [self.validate_standard_concept(item) for item in self.standard_concept] if self.standard_concept else None
    ...
    def validate_vocabulary(self, item: str) -> str:
        valid_vocabularies = ['SNOMED','MeSH','UCUM','OMOP Extension','LOINC','ATC','RxNorm','Gender','Race','Ethnicity']
...
def convert_to_documents(results) -> List[Document]:
    matching_documents = [
        Document(id=f"{str(item['vocabulary']).strip().lower()}:{str(item['code'])}", page_content=str(item['name']).lower(),
                 metadata={
