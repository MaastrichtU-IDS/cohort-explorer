...
the Decentriq enclave on the FULL cohort data and writes AGGREGATES ONLY:
figures (PNG), tables (CSV) and a provenance note. Every figure carries a
subtext stating exactly which mapping produced it.

Privacy rules baked into the script:
  - no row-level output ever; only counts, summaries and binned/hexbin densities
  - small-cell suppression: any count below k (spec "suppression_k", default 5)
    is suppressed, bins/cells below k are blanked, group summaries need n >= k
  - numeric extremes are reported as 5th/95th percentiles, never min/max
  - correlation shows coefficients, a suppressed hexbin density and binned means,
    never a scatter of individual points
...
ANALYSIS_KINDS = {
    ...
    "correlation": {
        ...
        "min_cohorts": 1,
        "max_cohorts": 1,
        "blurb": "Correlation coefficients with confidence intervals, a density map and binned means — never individual points.",
    },
    "crosstab": {
...
def describe_spec(spec: dict[str, Any]) -> str:
    ...
    parts.append(f"Small cells below {a.get('suppression_k', 5)} are suppressed.")
    return " ".join(parts)
...
# It reads the full cohort data inside the enclave and writes AGGREGATES ONLY:
# figures, summary tables and a provenance note. No row-level data leaves.
#
#   __DESCRIPTION__
###############################################################################
import csv
...
KIND = ANALYSIS["kind"]
K = int(ANALYSIS.get("suppression_k", 5))
...
def footer_text(used):
    ...
    head = "Mapping '%s' (by %s). Cells with n<%d suppressed." % (mname, who, K)
...
def fmt_count(n):
    return str(int(n)) if int(n) >= K else "<%d" % K
...
def numeric_summary(s, label):
    ...
    if n >= K:
        ...
        row.update({k: None for k in ["mean", "sd", "median", "q1", "q3", "p5", "p95"]})
    return row
...
def category_counts(s, label=None):
    ...
    for val, n in vc.items():
        rows.append({"group": label, "value": val, "count": fmt_count(n),
                     "percent": round(100.0 * n / len(s), 1) if n >= K and len(s) else None})
    return rows
...
def chi2_sf(x, df):
    """Upper tail of the chi-square distribution via the regularized upper
    incomplete gamma function (no scipy in the enclave)."""
    if x <= 0:
        return 1.0
    a = df / 2.0
    x = x / 2.0
    # series for P(a,x) when x < a+1, continued fraction for Q otherwise
    if x < a + 1:
...
def run_distribution(df, var, label, group_label=None, fname_prefix="distribution"):
    ...
    if hv.get("type") == "numeric":
        ...
        if counts is None:
            notes.append("%s: fewer than %d values, figure suppressed" % (var, K))
            return
        ...
        ax.set_ylabel("Patients (bins with n<%d blanked)" % K)
        ...
        if not shown:
            notes.append("%s: all categories below %d, figure suppressed" % (var, K))
            return
        fig, ax = plt.subplots(figsize=(9, 5.5))
    ...
    if hv.get("type") == "numeric":
        ...
        for g in groups:
            ...
            if len(s) >= K:
                ...
                counts = np.where(counts >= K, counts, 0).astype(float)
                dens = counts / max(counts.sum(), 1) / np.diff(edges)
                ax.step(edges[:-1], dens, where="post", label="%s (n=%d)" % (g, len(s)))
        save_table(pd.DataFrame(rows), fname_prefix + "_summary.csv", "%s by %s" % (var, group))
        ax.set_xlabel(hv.get("label") or var)
        ax.set_ylabel("Density (bins with n<%d blanked)" % K)
        ...
        ax.set_ylabel("Patients (cells with n<%d blanked)" % K)
        ax.set_title(plain_title(title))
        ax.legend(title=gv.get("label") or group)
        save_fig(fig, fname_prefix + ".png", used, "%s by %s" % (hv.get("label") or var, gv.get("label") or group))
...
def run_correlation(df, x, y, title, fname_prefix="correlation"):
    ...
    rows = [{"n": fmt_count(n)}]
    if n >= max(K, 4):
        r_p = float(sub[x].corr(sub[y], method="pearson"))
        r_s = float(sub[x].corr(sub[y], method="spearman"))
        lo, hi = fisher_ci(r_p, n)
        rows[0].update({"pearson_r": round(r_p, 3), "pearson_ci95_low": lo, "pearson_ci95_high": hi,
                        "spearman_rho": round(r_s, 3)})
    save_table(pd.DataFrame(rows), fname_prefix + "_coefficients.csv", "Correlation of %s and %s" % (x, y))
    if n < max(K, 4):
        notes.append("correlation: fewer than %d complete pairs, figure suppressed" % max(K, 4))
        return
    fig, axes = plt.subplots(1, 2, figsize=(12, 5.5))
    # density map, cells below K blanked
    hb = axes[0].hexbin(sub[x], sub[y], gridsize=18, mincnt=K, cmap="Blues")
    fig.colorbar(hb, ax=axes[0], label="patients per cell (cells with n<%d blank)" % K)
    ...
    try:
        ...
        binned.loc[~ok.values, "mean_y"] = None
    ...
    axes[1].set_title("Binned means (bins with n<%d dropped)" % K)
    fig.suptitle("%s  —  Pearson r=%.2f [%s, %s], Spearman rho=%.2f, n=%d" % (
        plain_title(title), rows[0].get("pearson_r", float("nan")), rows[0].get("pearson_ci95_low"),
        rows[0].get("pearson_ci95_high"), rows[0].get("spearman_rho", float("nan")), n))
...
def run_crosstab(df, x, y, title, fname_prefix="crosstab"):
    ...
    for i in table.index:
        for j in table.columns:
            n = ct.loc[i, j]
            table.loc[i, j] = ("%d (%.1f%%)" % (n, pct.loc[i, j])) if n >= K else "<%d" % K
    ...
    if ct.shape[0] > 1 and ct.shape[1] > 1 and ct.values.sum() >= K:
        expected = np.outer(ct.sum(axis=1), ct.sum(axis=0)) / ct.values.sum()
        with np.errstate(divide="ignore", invalid="ignore"):
            chi2 = float(np.nansum((ct.values - expected) ** 2 / expected))
        dof = (ct.shape[0] - 1) * (ct.shape[1] - 1)
        p = chi2_sf(chi2, dof)
        v = math.sqrt(chi2 / (ct.values.sum() * (min(ct.shape) - 1))) if min(ct.shape) > 1 else None
        stat_rows[0].update({"chi_square": round(chi2, 3), "dof": dof, "p_value": round(p, 4),
...
def run_compare(df, var, title, fname_prefix="compare"):
    ...
    if hv.get("type") == "numeric":
        ...
        for c in COHORTS:
            ...
            if len(s) >= K and edges is not None:
        ...
        ax.set_ylabel("% of cohort (cells with n<%d blanked)" % K)
        ax.set_title(plain_title(title))
        ax.legend()
        save_fig(fig, fname_prefix + ".png", used, "%s across cohorts" % (hv.get("label") or var))
...
def run_pooled(df, var, group, title, fname_prefix="pooled"):
    ...
    if hv.get("type") == "numeric":
        allv = pd.to_numeric(df[var], errors="coerce").dropna()
        if len(allv) < K:
            notes.append("pooled: fewer than %d values" % K)
            return
        ...
        ax.set_xlabel("%s%s" % (hv.get("label") or var, " (%s)" % hv.get("unit") if hv.get("unit") else ""))
        ax.set_ylabel("Patients, stacked by cohort (bins with n<%d blanked)" % K)
        ax.set_title(plain_title(title))
        ax.legend()
        ...
        ax.set_ylabel("Patients, stacked by cohort (cells with n<%d blanked)" % K)
        ax.set_title(plain_title(title))
        ax.legend()
        plt.setp(ax.get_xticklabels(), rotation=30, ha="right")
        save_fig(fig, fname_prefix + ".png", used, "Pooled distribution of %s" % (hv.get("label") or var))
