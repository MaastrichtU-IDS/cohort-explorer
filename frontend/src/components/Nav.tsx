...
export function Nav() {
  ...
  // The flexible wizard is the default; a subtle link at its bottom-left
  // switches to the no-code wizard (Beta).
  const [wizardMode, setWizardMode] = useState<'flexible' | 'nocode'>('flexible');
  ...
  // creator: they are always part of their own DCR (the backend refuses to
  // exclude them), so their own row stays ticked.
  useEffect(() => {
    if (!userEmail) return;
    const mine = dataOwners.find(o => o.email.toLowerCase() === userEmail.toLowerCase());
    if (mine && !manuallyIncludedOwners.includes(mine.email)) {
      setManuallyIncludedOwners([...manuallyIncludedOwners, mine.email]);
    }
            ...
            {!userEmail ? (
              ...
              <>
                ...
                <div className="mt-2 text-left">
                  <button
                    type="button"
                    className="btn btn-ghost btn-xs px-1 gap-1.5 text-base-content/50 hover:text-base-content font-normal"
                    onClick={() => setWizardMode('nocode')}
                  >
                    Or: switch to no-code DCRs
                    <span className="badge badge-warning badge-xs font-semibold uppercase tracking-wide">Beta</span>
                  </button>
                </div>
