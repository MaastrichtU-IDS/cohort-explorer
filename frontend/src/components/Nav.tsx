...
export function Nav() {
  ...
  const [dcrCohortInfo, setDcrCohortInfo] = useState<Record<string, {requiresProject: boolean; requiresResearchPurpose: boolean; diseaseCodes: string[]; modifiers: string[]}>>({});
  const [dcrProjectIds, setDcrProjectIds] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    if (!dcrProfileResult) return;
    const ids = Object.keys(dataCleanRoom?.cohorts || {});
    ids.forEach(cohortId => {
      if (dcrCohortInfo[cohortId]) return;
      fetch(`http://localhost:8020/api/cohorts/${encodeURIComponent(cohortId)}`)
        .then(r => (r.ok ? r.json() : null))
        .then(d => {
          setDcrCohortInfo(prev => ({...prev, [cohortId]: {
            requiresProject: !!d?.requiresProject,
            requiresResearchPurpose: !!d?.requiresResearchPurpose,
            diseaseCodes: d?.diseaseCodes ?? [],
            modifiers: d?.modifiers ?? [],
          }}));
        })
        .catch(() => { /* card falls back to optional project id */ });
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dcrProfileResult, dataCleanRoom?.cohorts]);
                    ...
                    <>
                      ...
                      <div className="form-control mb-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-3 border border-base-300 rounded-lg">
                          {([
                            { id: 'genetic',    duo: 'GRU', label: 'My research involves genetic or genomic studies' },
                            { id: 'ancestry',   duo: 'POA', label: 'My research involves population origins or ancestry analysis' },
                            { id: 'commercial', duo: 'GRU', label: 'This work is not part of any commercial product or service' },
                            { id: 'irb',        duo: 'GRU', label: 'Ethics approval has been obtained for this project' },
                            { id: 'publish',    duo: 'GRU', label: 'I intend to publish findings related to this analysis' },
                          ] as const).map(opt => (
                                ...
                                return (
                                  <div key={cohortId} className="border border-base-300 rounded-lg p-3 space-y-2">
                                    ...
                                    {!result && (
                                      <>
                                      {(() => {
                                        // Every selected code is sent as-is. Whether it is acceptable is decided by
                                        // icarechain and the chain, and the answer (including rejections) is recorded there.
                                        const selectedCodes = Array.from(new Set(dcrDiseaseCodes.map(e => e.code.trim().toUpperCase()))).sort();
                                        const info = dcrCohortInfo[cohortId];
                                        const projectMissing = !!info?.requiresProject && !(dcrProjectIds[cohortId] || '').trim();
                                        return (
                                          <>
                                            {info?.requiresProject && (
                                              <div className="form-control mb-2">
                                                <label className="label py-0"><span className="label-text text-xs font-semibold">Project ID <span className="text-error">*</span></span></label>
                                                <input type="text" className="input input-bordered input-sm" placeholder="Exactly as listed by the data provider"
                                                  value={dcrProjectIds[cohortId] || ''} onChange={e => setDcrProjectIds(prev => ({...prev, [cohortId]: e.target.value}))} />
                                                <label className="label py-0"><span className="label-text-alt text-base-content/60">This cohort restricts access to specific projects. Enter the project name or ID the provider registered.</span></label>
                                              </div>
                                            )}
                                            {projectMissing && <p className="text-xs text-warning mb-1">A project ID is required for this cohort.</p>}
                                      <button
                                        type="button"
                                        className="btn btn-accent btn-sm w-full"
                                        disabled={!!loading || projectMissing}
                                        onClick={async () => {
                                          setDcrCohortLoading(prev => ({ ...prev, [cohortId]: true }));
                                          try {
                                            // icarechain purposes: general | health | disease | population | methods | genetics | clinical.
                                            // GSO cohorts accept only "genetics"; NPOA cohorts reject "population".
                                            const researchPurpose = dcrIntendedUses.includes('genetic') ? 'genetics'
                                              : dcrIntendedUses.includes('ancestry') ? 'population'
                                              : (selectedCodes.length > 0 ? 'disease' : 'general');
                                            const projectId = (dcrProjectIds[cohortId] || '').trim();
                                            const accessRequestBody = {
                                              email: userEmail,
                                              cohortId,
                                              intendedUse: dcrIntendedUses.includes('ancestry') ? 'POA'
                                                : (selectedCodes.length > 0 ? 'DS' : 'GRU'),
                                              researchPurpose,
                                              diseaseCodes: selectedCodes.length > 0 ? selectedCodes : undefined,
                                    ...
                                    {result && (
                                      <div className="text-sm space-y-1">
                                        {result.error ? (
                                          ...
                                          <>
                                            <p>Status: <span className={`badge badge-xs ${result.matched || result.auto_approved ? 'badge-success' : result.decision === 'rejected' ? 'badge-error' : 'badge-warning'}`}>{result.matched || result.auto_approved ? 'Approved' : result.decision === 'rejected' ? 'Rejected' : (result.status || 'Pending')}</span></p>
                                            {result.decision === 'rejected' && (result.reason_detail || result.reason) && (
                                              <p className="text-xs text-error">{result.reason_detail || result.reason}{result.reason && result.reason_detail ? ` (${result.reason})` : ''}</p>
                                            )}
