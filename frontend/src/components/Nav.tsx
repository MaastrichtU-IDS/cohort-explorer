...
/** Subset of ICD-10 accepted by the icarechain API (generated by scripts/generate_icd10_json.py --chain). */
type Icd10Chain = {leaves: string[]; known: string[]; children: Record<string, string[]>};

/**
 * Mirror of icarechain's requester-side disease code validation
 * (api/routes/requesters.py::_validate_request_format): blocks/chapters are
 * rejected, a known code must be terminal, and an unknown code is rolled up
 * to the nearest known ancestor by stripping trailing characters after the dot.
 */
function resolveRequestDiseaseCodes(
  selected: {code: string; kind: string}[],
  chain: Icd10Chain | null,
): {codes: string[]; problems: string[]} {
  const problems: string[] = [];
  const codes: string[] = [];
  const leaves = new Set(chain?.leaves ?? []);
  const known = new Set(chain?.known ?? []);
  for (const entry of selected) {
    const c = entry.code.trim().toUpperCase();
    if (entry.kind !== 'category' || /-/.test(c)) {
      problems.push(`${c}: blocks and chapters are not allowed for requests; pick a specific disease code.`);
      continue;
    }
    if (!chain) { codes.push(c); continue; }
    if (known.has(c)) {
      if (!leaves.has(c)) {
        const kids = (chain.children[c] ?? []).join(', ');
        problems.push(`${c} is not terminal in the supported hierarchy; use a more specific code${kids ? ` (${kids})` : ''}.`);
        continue;
      }
      codes.push(c);
      continue;
    ...
    let r = c;
    while (!known.has(r)) {
      if (!r.includes('.')) { r = ''; break; }
      r = r.slice(0, -1).replace(/\.$/, '');
    }
    if (!r) {
      problems.push(`${c} is not in the ICD-10 subset supported by the consent chain.`);
      continue;
    }
    codes.push(r);
  }
  const uniq = Array.from(new Set(codes)).sort();
  return {codes: uniq, problems};
}
...
export function Nav() {
  ...
  const [dcrIcd10Chain, setDcrIcd10Chain] = useState<Icd10Chain | null>(null);
  ...
  useEffect(() => {
    if (wizardStep === 2 && dcrIcd10Entries.length === 0 && !dcrIcd10Loading && !dcrIcd10LoadFailed) {
      ...
      fetch('/icd10-chain.json')
        .then(r => (r.ok ? r.json() : null))
        .then((chain: Icd10Chain | null) => { if (chain) setDcrIcd10Chain(chain); })
        .catch(() => { /* validation then falls back to the API's own error message */ });
      fetch('/icd10.json')
                                ...
                                return (
                                  <div key={cohortId} className="border border-base-300 rounded-lg p-3 space-y-2">
                                    ...
                                    {!result && (
                                      <>
                                      {(() => {
                                        const {codes: resolvedCodes, problems} = resolveRequestDiseaseCodes(dcrDiseaseCodes, dcrIcd10Chain);
                                        const info = dcrCohortInfo[cohortId];
                                        const projectMissing = !!info?.requiresProject && !(dcrProjectIds[cohortId] || '').trim();
                                        return (
                                          <>
                                            {problems.length > 0 && (
                                              <ul className="text-xs text-warning mb-1 list-disc pl-4 space-y-0.5">
                                                {problems.map(pr => <li key={pr}>{pr}</li>)}
                                              </ul>
                                            )}
                                            {problems.length === 0 && resolvedCodes.length > 0 && resolvedCodes.join(',') !== dcrDiseaseCodes.map(e => e.code.toUpperCase()).sort().join(',') && (
                                              <p className="text-xs text-base-content/60 mb-1">Will be submitted as: <code>{resolvedCodes.join(', ')}</code></p>
                                            )}
                                      ...
                                      <button
                                        ...
                                        disabled={!!loading || problems.length > 0 || projectMissing}
                                        onClick={async () => {
                                          ...
                                          try {
                                            ...
                                            const researchPurpose = dcrIntendedUses.includes('genetic') ? 'genetics'
                                              ...
                                              : (resolvedCodes.length > 0 ? 'disease' : 'general');
                                            const projectId = (dcrProjectIds[cohortId] || '').trim();
                                            const accessRequestBody = {
                                              email: userEmail,
                                              cohortId,
                                              intendedUse: dcrIntendedUses.includes('ancestry') ? 'POA'
                                                : (resolvedCodes.length > 0 ? 'DS' : 'GRU'),
                                              researchPurpose,
                                              diseaseCodes: resolvedCodes.length > 0 ? resolvedCodes : undefined,
